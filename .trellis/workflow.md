...
6. **No sub-agents** — sub-agent dispatch is prohibited on every platform; all work happens in the main session
7. **Tasks only on request** — never create a Trellis task unless the user explicitly asks for one; default to working directly
...
- **Default: no Trellis task.** Do not create a task unless the user explicitly asks for one; work directly in the main session.
- Only when the user explicitly asks for a task: create it (Phase 1.0) and enter planning. Task-creation approval is still not implementation approval — planning happens first.
- **Sub-agents are prohibited on every platform**: plan, implement, and check inline by loading `trellis-*` skills in the main session.
...
No active task. Do NOT create a Trellis task unless the user explicitly asks for one; work directly in the main session.
Sub-agents are prohibited on every platform: never dispatch implement/check/research sub-agents; do the work inline.
If the user explicitly asks for a task: create it, then stay in planning until artifacts are reviewed.
[/workflow-state:no_task]

### Phase 1: Plan
- 1.0 Create task `[required · once]` (only when the user explicitly asks for a task)
- 1.1 Requirement exploration `[required · repeatable]` (`prd.md`; complex tasks also need `design.md` + `implement.md`)
- 1.2 Research `[optional · repeatable]`
- 1.3 Configure context `[skipped in this project]` — sub-agents are prohibited here, jsonl manifests are never consumed; skip this step entirely
...
Sub-agents are prohibited in this project: skip jsonl curation; read specs and research directly in the main session.
[/workflow-state:planning]
...
Sub-agents are prohibited in this project on every platform; the sub-agent dispatch protocol does not apply. All work happens in the main session.

[workflow-state:in_progress]
Sub-agents are prohibited: do NOT dispatch `trellis-implement` / `trellis-check` / `trellis-research` (or any other) sub-agents on any platform; do all work inline in the main session.
...
- `in_progress` implementation/check -> `trellis-before-dev` before editing, `trellis-check` skill after; sub-agents are prohibited in this project.
...
- Never create a Trellis task unless the user explicitly asks for one; the default is to work directly in the main session.
- Never dispatch sub-agents (`trellis-implement` / `trellis-check` / `trellis-research` or any Task/Agent spawn); all work happens inline.
...
Goal: classify the request, create a task only when the user explicitly asks for one, and produce the planning artifacts required before implementation.

#### 1.0 Create task `[required · once]`

Create the task directory only when the user explicitly asks for a Trellis task — never proactively offer or create one. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
**Sub-agents are prohibited in this project** — do the research in the main session directly and write findings into `{TASK_DIR}/research/`; the dispatch block below does not apply.
...
#### 1.3 Configure context `[skipped in this project]`

> **Project rule: sub-agents are prohibited.** `implement.jsonl` / `check.jsonl` are never consumed and the jsonl ready-gate in 1.4 does not apply. The platform blocks below are kept as template reference only — skip them.
...
For lightweight tasks, `prd.md` can be enough. For complex tasks, `prd.md`, `design.md`, and `implement.md` must exist and be reviewed before start. The jsonl ready-gate does not apply in this project (sub-agents are prohibited; the manifests are never consumed).
...
| `implement.jsonl` and `check.jsonl` each contain at least one real curated entry (seed row does not count) | — (skipped: sub-agents prohibited) |
...
> **Project rule: sub-agents are prohibited on every platform.** Ignore the dispatch blocks below; always follow the inline variant (`[codex-inline, Kilo, Antigravity, Devin]`).
...
> **Project rule: sub-agents are prohibited on every platform.** Ignore the dispatch block below; load the `trellis-check` skill in the main session (inline variant).
...
- No active task must not create a Trellis task unless the user explicitly asks for one; sub-agent dispatch is prohibited on every platform.
